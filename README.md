...
A file explorer, inspector, and light editor for [Möbius](https://github.com/mobius-os) — a MiXplorer-style window into everything under `/data`. Browse the server by drilling into folders, sort and filter what is there, inspect any file or folder's full metadata, preview images, edit text and markdown in place, and summon an embedded agent to make edits with you.
...
- **Drill-in navigation** — tap a folder to go into it; a tappable breadcrumb (`/data › apps › notes`) jumps to any ancestor, and the Up button ascends one level. Listings are lazy (one directory level at a time, paginated, cached) so revisits and breadcrumb jumps are instant.
- **Detail list** — dense rows with the file-kind glyph, name, and a metadata line (size · relative-modified, or item-count for folders). The timestamp carries recency directly without adding competing status markers.
- **Sort, filter, view** — sort by name / size / modified / kind (asc↔desc, folders-first), show or hide dotfiles, filter the current folder by name, and switch between a dense **list** and an image-friendly **grid** (viewport-gated, byte-capped, concurrency-limited lazy thumbnails). Visual preferences persist via `window.mobius.storage`; folder tabs are session-scoped, and every fresh launch begins in `apps` with hidden files off by default.
- **Properties inspector** — the deep view for any file or folder: full path (copyable), kind + MIME, exact size (or immediate item count for a folder), modified time (absolute + relative), owner-editable vs platform-managed, and text vs binary. Delete lives here (files only).
...
- **Embedded agent** — an on-demand `window.mobius.chat` sheet (bottom sheet on phone, docked panel on desktop), pre-seeded with the current path and open-file context. After each agent turn the open file and listing refresh live.
...
| File | Role |
|------|------|
| `index.jsx` | App orchestrator: navigation, listing/sort/filter/view state, the open-file + save state machine, overlays, and composition. |
| `constants.js` | Scalar constants (FS root, extension sets, view/sort keys, prefs defaults, shortcuts, listing caps). |
| `paths.js` | Pure, dependency-free path / name / format / sort helpers (relative time, breadcrumb segments, `sortEntries`, recents, and save-state helpers). Unit-tested. |
...
| `ui/CodeEditor.jsx` `ui/Preview.jsx` `ui/ChatPanel.jsx` `ui/NameModal.jsx` `ui/ConfirmModal.jsx` `ui/useModalFocus.js` `ui/ChatBubbleIcon.jsx` | Editor, previews, chat embed, and modals. |

## Data

- **`ui-prefs.json`** (via `window.mobius.storage`) — view mode, sort key + direction, folders-first, hidden-file visibility, pinned bookmarks, and recents. Folder locations and tabs stay session-scoped.
...
Unit tests (pure helpers plus save-state and sort/format regressions) run on a fresh clone with zero install — Node's built-in test runner:
