...
- **Detail list** — dense rows with the file-kind glyph, name, and a metadata line (size · relative-modified, or item-count for folders). A subtle dot marks anything changed in the last 24 h; repo folders are badged `git`; files the agent touched carry a git change chip inline.
...
- **Git banner** — on entering a repo, a collapsible bar shows branch, ahead/behind, and staged/modified/untracked counts; expand it to open any changed file. This is the "inspect what the agent did" loop.
...
- **CodeMirror editor** — a writable text file opens directly editable (writability drives editability — no mode toggle); markdown gets the live-preview with emphasis, headings, task checkboxes, and `$…$`/`$$…$$` KaTeX math. Read-only platform files show the preview with a badge. Large files (>5 MB) peek their first 256 KB read-only.
